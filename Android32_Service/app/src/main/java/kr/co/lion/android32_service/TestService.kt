...
// 이 때, AndroidManifest.xml에 FOREGROUND_SERVICE 권한을 등록해야 한다.
class TestService : Service() {
    ...
    // 쓰래드의 반복문의 조건식으로 사용할 변수
    var isRunning = false
    var value = 0

    // 외부에서 서비스에 접속하면 호출되는 메서드
    // 여기에서 Binder 객체를 반환하면 Binder를 객체를 Activity에서 받을 수 있다.
    // 반환한 값을 OS가 가지고 있음
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            ...
            // 알림메시지를 띄운다
            // 안드로이드 8.0 부터는 서비스 가동시 알림 메시지를 띄워야 한다.
            // 안드로이드 10.0 에서 서비스에 대한 용도를 지정하는 개념이 추가됬다.
            ...
            // 이 때, 용도를 명시했고 구글로 부터 인증을 받았기 때문에 알림 메시지를 띄우지 않는다.
            // 안드로이드 14 버전 부터 서비스의 용도를 명시하는 것이 의무화 되었다. <- 구글이 확인하겠다는 뜻
