...
// SQLiteOpenHelper의 생성자의 매개변수
// 첫 번째 : 컨텍스트
// 두 번째 : 데이터 베이스 파일의 이름. 이 이름으로 만들어진 데이터 베이스 파일이 없으면 OS가 파일을 만들어주고
// OS가 onCreate 메서드를 호출한다.
// 세 번째 : null에 대한 처리를 어떻게 할 것인가. 그냥 null 셋팅해주세요
// 네 번째 : 데이터 베이스 파일의 버전. 애플리케이션의 버전과 무관하며 아무 숫자나 지정해 주면 된다. 애플리케이션을 서비스 하는 중에
// 테이블의 개수나 구조를 변경해야 하는 경우 이전의 숫자보다 높은 숫자로 지정해주면 예전에 저장한 애플리케이션들은 onUpgrade를 호출해준다.
...
class DBHelper(context:Context, dbName:String, version:Int) : SQLiteOpenHelper(context, dbName, null, version) {
    ...
    // 각 행들을 개별적으로 구분하기 위한 값을 저장하기 위해 사용한다.
    // autoincrement : 컬럼에 저장할 값을 지정하지 않으면 1부터 1씩 증가되는 값이 자동으로 저장된다.
    // not null : null을 허용하지 않는다. 즉, 개발자가 무조건 값을 정해줘야 한다.
