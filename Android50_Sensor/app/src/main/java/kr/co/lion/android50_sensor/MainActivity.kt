...
class MainActivity : AppCompatActivity() {
    ...
    // 마그네틱 필드 센서의 리스너를 담을 프로퍼티
    var magenticSensorListener:MagenticSensorListener? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityMainBinding.apply {
            ...
            button6.setOnClickListener {
                // 리스너 연결이 되어 있지 않을 경우
                if(magenticSensorListener == null){
                    button6.text = "마그네틱 필드 센서 해제"
                    // 리스너 객체를 생성
                    magenticSensorListener = MagenticSensorListener()
                    // 센서들을 관리하는 객체를 추출한다.
                    val sensorManager = getSystemService(SENSOR_SERVICE) as SensorManager
                    // 근접 센서 객체를 가져온다.
                    ...
                    // 세 번째 : 데이터를 받아올 주기
                    // 반환값 : 센서 연결에 성공했는지 여부.
                    val chk = sensorManager.registerListener(magenticSensorListener, sensor, SensorManager.SENSOR_DELAY_UI)
                    // 센서 연결에 실패했다면
                    if(chk == false){
                        magenticSensorListener = null
                ...
                // 리스너가 연결되어 있을 경우
                    ...
                    val sensorManager = getSystemService(SENSOR_SERVICE) as SensorManager
                    // 가속도 센서의 리스너를 해제한다.
                    sensorManager.unregisterListener(magenticSensorListener)
    ...
    // 마그네틱 필드 센서의 리스너
    inner class MagenticSensorListener : SensorEventListener{
        // 센서에 변화가 일어날 때..
        // 이 메서드에서 측정된 값을 가져올 수 있다
        override fun onSensorChanged(event: SensorEvent?) {
            if(event != null) {
